...
describe("gammaToMarket", () => {
  ...
  it("infers resolvedOutcome from settled prices on a closed market", () => {
    const m = gammaToMarket(raw({ closed: true, outcomePrices: '["1", "0"]' }))!;
  ...
  it("does not derive bid/ask for non-binary markets", () => {
    const m = gammaToMarket(
      raw({ outcomes: '["A","B","C"]', outcomePrices: '["0.2","0.3","0.5"]', bestBid: 0.2, bestAsk: 0.25 }),
    )!;
    expect(m.outcomes.every((o) => o.bid === null && o.ask === null)).toBe(true);
  });
});
