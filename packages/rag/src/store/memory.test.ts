...
import { InMemoryVectorStore, cosineSimilarity } from "./memory.js";
import type { VectorRecord } from "./vector-store.js";

function makeChunk(id: string): Chunk {
  return { id, evidenceId: `ev-${id}`, text: `text ${id}`, index: 0, tokenCount: 1, publishedAt: null };
}

function makeRecord(id: string, embedding: number[], marketId?: string): VectorRecord {
...
describe("InMemoryVectorStore", () => {
  ...
  it("restricts results to a marketId when one is given", async () => {
    ...
    // Act
    const hits = await store.query({ embedding: [1, 0], topK: 10, marketId: "mkt-1" });
