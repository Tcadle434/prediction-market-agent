...
describe("recursiveChunker", () => {
  ...
  it("returns a single chunk when the document fits the budget", () => {
    const chunks = recursiveChunker({ targetTokens: 100 }).chunk(evidenceWith("short text"));
  ...
  it("breaks on paragraph boundaries when whole paragraphs fit", () => {
    ...
    // Act
    const chunks = recursiveChunker({ targetTokens: 10 }).chunk(evidenceWith(content));
  ...
  it("recurses into an oversized paragraph and stays within budget", () => {
    ...
    // Act
    const chunks = recursiveChunker({ targetTokens }).chunk(evidenceWith(content));
  ...
  it("is deterministic", () => {
    ...
    const evidence = evidenceWith("alpha bravo. charlie delta. echo foxtrot. golf hotel india.");
